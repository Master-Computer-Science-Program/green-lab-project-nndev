...
dat_data <- read.csv("../benchmarks/experiments/run_table.csv") %>%
  select(guideline, code, treatment, run_number, execution_time, execution_time, execution_time, memory_usage) %>%
  mutate(
...
```{r}
p <- ggplot(dat_data, aes(x = execution_time)) +
...
ggsave("figures/execution_time_histogram.png", plot = p, width = 8, height = 6, dpi = 300)
```

#### 2.1.2. Split by treatment
```{r}
p <- ggplot(dat_data, aes(x = execution_time, fill = treatment)) +
...
ggsave("figures/execution_time_histogram_by_treatment.png", plot = p, width = 8, height = 6, dpi = 300)
```

### 2.2. Box plot of execution_time grouped by treatment and guideline
```{r}
p <- ggplot(dat_data, aes(x = treatment, y = execution_time, fill=guideline)) +
...
ggsave("figures/execution_time_box_plot.png", plot = p, width = 8, height = 6, dpi = 300)
```
...
p <- ggplot(dat_data, aes(x = run_number, y = execution_time, color = experiment)) +
...
ggsave("figures/execution_time_scatterplot.png", plot = p, width = 8, height = 6, dpi = 300)
```
