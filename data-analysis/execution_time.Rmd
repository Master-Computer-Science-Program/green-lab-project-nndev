...
dat_data <- dat_data %>%
  dplyr::filter(!is.na(execution_time) & !is.na(guideline) & !is.na(treatment))
...
### 2.3 Scatterplots
#### 2.3.1 Scatterplots of execution_time across trials: define "experiment" as guideline + treatment
```{r}
dat_data <- dat_data %>%
  mutate(
...
#### 2.3.2 Scatterplots of memory_usage across trials (by code)
```{r}
dat_data <- dat_data %>%
  mutate(
    experiment = interaction(guideline, treatment),
...
wilcox_results_effsize <- map_dfr(guidelines, function(g) {
  ...
  # Cliff's Delta
  cliff_result <- cliff.delta(guideline_j, no_guideline_j)
