...
dat_data <- dat_data %>%
  dplyr::filter(!is.na(memory_usage) & !is.na(guideline) & !is.na(treatment))
...
#### 2.3.1 Scatterplots of memory_usage across trials: define "experiment" as guideline + treatment
```{r}
dat_data <- dat_data %>%
  mutate(
...
wilcox_results_effsize <- map_dfr(guidelines, function(g) {
  ...
  no_guideline_j <- subset_data %>% dplyr::filter(treatment == "no_guideline") %>% pull(memory_usage)
  guideline_j     <- subset_data %>% dplyr::filter(treatment == "guideline") %>% pull(memory_usage)
