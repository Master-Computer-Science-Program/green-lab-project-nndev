...
```{r}
p <- ggplot(dat_data, aes(x = energy_usage)) +
...
ggsave("figures/energy_usage_histogram.png", plot = p, width = 8, height = 6, dpi = 300)
```

#### 2.1.2. Split by treatment
```{r}
p <- ggplot(dat_data, aes(x = energy_usage, fill = treatment)) +
...
ggsave("figures/energy_usage_histogram_by_treatment.png", plot = p, width = 8, height = 6, dpi = 300)
```

### 2.2. Box plot of energy_usage grouped by treatment and guideline
```{r}
p <- ggplot(dat_data, aes(x = treatment, y = energy_usage, fill=guideline)) +
...
ggsave("figures/energy_usage_box_plot.png", plot = p, width = 8, height = 6, dpi = 300)
```
...
p <- ggplot(dat_data, aes(x = run_number, y = energy_usage, color = experiment)) +
...
ggsave("figures/energy_usage_scatterplot.png", plot = p, width = 8, height = 6, dpi = 300)
```
