...
all_no_guideline = dat_data[dat_data$treatment == 'no_guideline',]$energy_usage
all_guideline = dat_data[dat_data$treatment == 'guideline',]$energy_usage
wilcox.test(all_no_guideline, all_guideline)
...
wilcox_results <- map_dfr(guidelines, function(g) {
  ...
  no_guideline <- subset_data %>% filter(treatment == "no_guideline") %>% pull(energy_usage)
  guideline     <- subset_data %>% filter(treatment == "guideline") %>% pull(energy_usage)
  
  # Run Wilcoxon test (only if both groups have data)
    test_result <- wilcox.test(no_guideline, guideline)
