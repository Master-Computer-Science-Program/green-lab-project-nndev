...
dat_data <- read.csv("../benchmarks/experiments/run_table.csv") %>%
  select(guideline, code, treatment, run_number, cpu_usage, execution_time, cpu_usage, memory_usage) %>%
  mutate(
...
dat_data <- dat_data %>%
  filter(!is.na(cpu_usage) & !is.na(guideline) & !is.na(treatment))
...
```{r}
p <- ggplot(dat_data, aes(x = cpu_usage)) +
...
ggsave("figures/cpu_usage_histogram.png", plot = p, width = 8, height = 6, dpi = 300)
```

#### 2.1.2. Split by treatment
```{r}
p <- ggplot(dat_data, aes(x = cpu_usage, fill = treatment)) +
...
ggsave("figures/cpu_usage_histogram_by_treatment.png", plot = p, width = 8, height = 6, dpi = 300)
```

### 2.2. Box plot of cpu_usage grouped by treatment and guideline
```{r}
p <- ggplot(dat_data, aes(x = treatment, y = cpu_usage, fill=guideline)) +
...
ggsave("figures/cpu_usage_box_plot.png", plot = p, width = 8, height = 6, dpi = 300)
```
...
p <- ggplot(dat_data, aes(x = run_number, y = cpu_usage, color = experiment)) +
  geom_point(alpha = 0.8, size = 2) +
  theme_minimal() +
  labs(
    title = "CPU Usage Across Runs",
    x = "Run Number",
    y = "CPU Usage",
    color = "Experiment"
  )

ggsave("figures/cpu_usage_scatterplot.png", plot = p, width = 8, height = 6, dpi = 300)
```
...
wilcox_results <- map_dfr(guidelines, function(g) {
  ...
  no_guideline_j <- subset_data %>% filter(treatment == "no_guideline") %>% pull(cpu_usage)
  guideline_j     <- subset_data %>% filter(treatment == "guideline") %>% pull(cpu_usage)
