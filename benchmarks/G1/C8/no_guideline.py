...
def main():
    ...
    reading_frames = [
        (1, tuple(map(str_to_bits, ('G', 'A', 'T', 'C')))),
        (2, tuple(map(str_to_bits, tuple(n + m
        for n in ('G', 'A', 'T', 'C') for m in ('G', 'A', 'T', 'C'))))),
    ] + list(map(lambda s: (len(s), (str_to_bits(s),)), ('GGT', 'GGTA', 'GGTATT', 
    ...
    display(results, display_list(('G', 'A', 'T', 'C')), relative=True, sort=True)
    display(results, display_list(tuple(n + m
        for n in ('G', 'A', 'T', 'C') for m in ('G', 'A', 'T', 'C'))), relative=True, sort=True)
    display(results, display_list(('GGT', 'GGTA', 'GGTATT', 
                                   'GGTATTTTAATT', 'GGTATTTTAATTTATAGT')), end='')
