...
class Chaosgame(object):
    ...
    def transform_point(self, point, trafo=None):
        ...
        var = 1 / 50000
        if t + var > end:
            neighbour = self.splines[trafo[0]](t - var)
...
if __name__ == "__main__":
    ...
    chaos = Chaosgame(splines, float(C3_ARG[0]) if len(C3_ARG) > 1 else DEFAULT_THICKNESS)  # thickness
    chaos.create_image_chaos(
        int(C3_ARG[1]) if len(C3_ARG) > 2 else DEFAULT_WIDTH,
        int(C3_ARG[2]) if len(C3_ARG) > 3 else DEFAULT_HEIGHT,
        int(C3_ARG[3]) if len(C3_ARG) > 4 else DEFAULT_ITERATIONS,
        C3_ARG[4] if len(C3_ARG) > 5 else None,
        int(C3_ARG[5]) if len(C3_ARG) > 6 else DEFAULT_RNG_SEED)  # rng_seed
