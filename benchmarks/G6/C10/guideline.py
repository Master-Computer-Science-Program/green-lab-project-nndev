...
class WorkTask(Task):
    ...
    def fn(self, pkt, r):
        ...
        for i in range(4):  # range(BUFSIZE)
            w.count += 1
            if w.count > 26:
                w.count = 1
...
class Richards(object):
    ...
    def run(self, iterations):
        for i in range(iterations):
            ...
            IdleTask(1, 1, 10000, TaskState().running(), IdleTaskRec())

            wkq = Packet(None, 0, 1001)
            wkq = Packet(wkq, 0, 1001)
            WorkTask(2, 1000, wkq, TaskState(
            ).waitingWithPacket(), WorkerTaskRec())

            wkq = Packet(None, 5, 1000)
            wkq = Packet(wkq, 5, 1000)
            wkq = Packet(wkq, 5, 1000)
            HandlerTask(3, 2000, wkq, TaskState(
            ).waitingWithPacket(), HandlerTaskRec())

            wkq = Packet(None, 6, 1000)
            wkq = Packet(wkq, 6, 1000)
            wkq = Packet(wkq, 6, 1000)
            HandlerTask(4, 3000, wkq, TaskState(
            ).waitingWithPacket(), HandlerTaskRec())

            wkq = None
            DeviceTask(5, 4000, wkq,
                       TaskState().waiting(), DeviceTaskRec())
            DeviceTask(6, 5000, wkq,
                       TaskState().waiting(), DeviceTaskRec())
